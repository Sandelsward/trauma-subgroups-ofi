...
treated. For example the rate of preventable deaths at the Karolinska University hospital trauma center was shown in a study to be 4.0 % [@Ghorbani2018-dh].  It has also been shown that trauma patients in Sweden who were treated at a trauma center rather than a non-trauma center had a 41 % lower 30-day adjusted mortality rate. [@Candefjord2022-pe] <!-- Lagt till hur set ser ut på KS här -->
...
The New Injury Severity Score (NISS) is developed from ISS. NISS also takes injuries from the same body regions as NISS into account. The NISS is however the sum of the squares of the three highest AIS codes in these regions, meaning that all three injuries can be in the same region or in separate regions. As the AIS can vary from 1-5, the NISS ranges between 1-75.[@Li2021-ag]
...
The patients were sorted into the the subgroups both by creating suitable columns and by using existing columns in the merged patient file. Gender, trauma mechanism, age, and blunt/ penetrating trauma all had existing column. New columns were created for OFI, minor/major trauma and for the most severe injured body region.
...
The OFI column was created using a pre existing function...?
...
Study size 
----------
The study size was arrived at in the first step by being limited by the number of patients in the selected registries. Then a complete case analysis was done on all patients in the registers.<!--Är det antal patienter som jag ska beskriva hur vi kom fram till?-->
...
BEskriva NISS, pentererande, trubbigt, kön,  
googla vilka är kategoriska och kontinuerliga osv
...
Statistical methods
-------------------
For handling the data and making the statistical analysis the program R was used. R is a programming language udes for statistical computing. The unfiltered data sets were uploaded to Rstudio which is an integrated development environment for R. Then all data that was not needed for this study was removed, including columns with non essential information, patients without complete data, and those who did not meet up with the inclusion criteria. New columns were also added as listed above. 
...
For estimating the confidence interval Bootstrapping was used on the data. To be able to do the Bootstrapping the absolute difference in OFI frequency within the subgropus was calculated. Meaning that the absolute difference between men and women was calculated but not for example between women and blunt trauma. 
...
Samma siffror som table 1. Hur ska jag göra med delta här? Kan jag göra något mer innan jag gjort bootstrapping?
