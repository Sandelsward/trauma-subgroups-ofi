...
Audit filters can be described as predefined statements that decides whether or not a certain event differ from the guidelines.[@Berg2022] Such audit filter can be time frames in which a test should be taken or a Glascow Coma Scale score under 8 without the patient being indubated[@Evans2009-oy]. 
 ...
 Trauma Svererity scores and systems (utbildningsprogram)
------------
...
<!--men bara trauma 1 till traumaenheten? trauma 2 till IA? något lite vagare -->
...
The column for Minor and Major trauma was creating using the NISS column. Patients with a NISS > 15 were defined as major trauma and those with NISS < 15 were defined as minor trauma. <!-- inte ha med detta trots att det är en ny kolumn? räcker att skriva att vi kategoriserade utifrån NISS-->
...
Study size 
----------
A complete case analysis was done on all patients in the registers that met the inclusion criteria for this study.
...
Out of the `r total.cohort` patients included in the cohort `r all.men` were men.  Among the men the frequency of OFI was `r freq.ofi.men` and among the women `r freq.ofi.women`. The mean age for all the patients were `r mean.age` years and the avarege NISS score was `r mean.niss`. 
...
Main results
------------
The subgroup with the highest OFI frequency was xxxx with a frequency at `r max.freq`%. The lowest OFI frequency was measured in the xxx cohort with a frquency at `r min.freq`%. In the entire cohort there was a `r mean.ofi`% frequency of OFI. 
```{r, echo = FALSE} 
  main.results ## will be improved later
```
