...
Introduction: Trauma represents 9% of the annual global deaths and is the most common cause of death in people <45 years. Trauma patients constitute a heterogeneous group of patients with a large variety of injuries. With this variety comes a complexity that demands specific guidelines for treating all kinds of different trauma subgroups. It is however poorly understood how the potential for trauma care improvement varies between these subgroups. One measure for this is Opportunity for improvement (OFI). Aims: In this study we assessed the OFI frequency in the clinically important trauma subgroups; men and women, blunt and penetrating trauma, minor and major trauma, and most severely injured body region. Material and methods: This was a registry based cohort study using the trauma registry and the trauma quality database at the Karolinska University hospital in Solna. 6146 patients included in the registries between 20xx-2021 met the inclusion criteria. 5718<!-- Are you sure about this number?--> patients were excluded due to either age <15 years or because of missing necessary information. Results:<!-- I would move the details about the number of patients included to the Results section-->  The mean OFI in the total cohort was 7%.<!-- These groups are not exclusive, so when you list OFI frequencies in abdominal trauma and major trauma, for example, the same patients may be present in both. It would be more fair to say that "looking at the most seriously injured body region, the frequency of OFI was highest among patients with abdominal trauma", and so on, so that you are alway explicit about your reference group. Again "Patients with major trauma had significantly more OFI than those with minor trauma, X vs Y% (95% CI and here is the delta CI)"--> It was found that patients with abdominal trauma had the highest frequency of OFI of all subgroups at 12,73% (CI:`r ci.abdomen`). In the binary subgroups blunt trauma at 7,08% and major trauma at 12,2% (CI: `r ci.major.trauma`) had the highest OFI. Conclusion: Our results suggest that OFI varies between trauma subgroups and that some subgroups have significantly higher OFI than baseline<!-- Is it really baseline or rather compared with some other subgroup?-->. Further research is however needed to identify what kind of OFIs that are more frequent in different cohorts as well as to identify risk factors for  OFI.
...
The Injury Severity Score (ISS) is an anatomic severity scale developed to assess multiple-injured patients. The ISS score assigned to a patient is calculated based on the AIS points from the injured body regions. The higher ISS score the more serious trauma the patients suffers from. The ISS does however only take into account the most severe injury in each region, meaning that you could have two different injuries in the abdomen without it affecting the ISS score.[@Javali2019-wn]

The New Injury Severity Score (NISS) was developed from the ISS. NISS also takes injuries from the same body regions as NISS<!-- Do you mean ISS here?--> into account. The NISS is the sum of the squares of the three highest AIS codes in these regions, meaning that all three injuries can be in the same region or in separate regions<!-- Maybe you should explain how ISS is calculated above, with the sum of squares, before you explain how NISS is calcualted?--> . As the AIS can vary from 1-5, the NISS ranges between 1-75 [@Li2021-ag]. The NISS score is also used internationally to define trauma severity. A NISS Score >15 is considered a major trauma and <15 a minor trauma [@Bagher2017].
...
sufficient understanding of OFIs for the different trauma subgroups.[@Coats2020-wj]. Some examples of subgroups are men and women, blunt and penetrating trauma, geriatric, major and minor trauma and traumatic brain injury [@arsrapportswetrau].<!-- I suggest that you try to expand on this, as it's the basis of your thesis. Ska jag lägga till exempel på skillnad i preventable death mellan subgrupperna här kanske? Flyttar up lite från diskussionen--> 
...
<!-- See my comment above. You need to work more to tie it all together and I think elaborating on the rationale and potential implications of your study is key to that. So I suggest that you focus on why your study is important and what could be the changes in practice that it may result in.-->
...
The most severely injured body region was identified using the AIS codes of each patient's injuries. For patients who had two or more different injuries and thereby multple AIS codes, the most severe injury was selected.
...
OFI  was treated as a binary variable, with "yes" and "no" as the only possible outcome. For the M&M conference to put a "yes" in the OFI section a consensus decision is required regarding the presence of at least one OFI.<!-- I suggest that you merge this with the first paragraph-->
...
----------------------
<!-- Quantitative means continuous in this case, so reduce duplication by moving information about categorical variables from here to the Variables section-->
The registers hold both categorical and continuous variables. NISS, age and AIS codes were treated as continuous variables. Penetrating/blunt trauma, minor/major trauma and OFI was treated as binary categorical variables. Most severely injured body region was treated as categorical variable.

The range of NISS was 1-75. Penetrating/blunt Trauma had the values of 1 and 2. 1 represented Blunt trauma and 2 penetraiting trauma. Minor/Major was derived from the NISS value. NISS>15 was considered a Major Trauma and <15 a Minor Trauma. OFI had the values of either YES or NO.

The most severe region injured had 10 different values: head, neck, face, thorax, upper extremity, lower extremity, abdomen, spine, multiple and unspecified. These 10 values were regrouped according to the ISS system into the following 7 cohorts."Head and neck", "Face",  "Chest, diaphragm or spine", "Abdomen", "Extremities", "Unspecified" and "Multiple".
...
<!-- Do you need to mention ethics here?-->
...
As shown in table 1, out of the `r total.cohort` patients included in the cohort `r all.men` (`r share.men`%) were men. The mean age for all patients were `r mean.age` years and the mean NISS score was `r mean.niss` (SD `r sd.niss`). There was `r tot.major.trauma` (`r share.major.trauma`%) cases of major trauma and the most frequent dominating type of trauma was blunt trauma with `r total.blunt.trauma` (`r share.blunt`%) cases. Among the cohort of most severely injured region head trauma was most frequent with a number of `r tot.head.neck` (`r share.head.neck`%) cases. The largest diffrence in number of OFI patients and no OFI patients was found in the xxx cohort<!-- I don't understand this sentence, what do you mean by larges difference in number of OFI patients? Number of patients with OFI, i.e. frequency? Use the word frequency instead then-->. The median age was higher in the OFI cohort at 47 years, compared with the non OFI cohort which had a median age of 42 years. Those who did not have OFI were more likely to be alive after 30 days (92,1%) compared to those in which OFI were identified (90,1%).
...
Main results
------------
In the entire sample the mean frequency of OFI was `r mean.ofi`%. The subgroup with the highest OFI frequency was the subgroup with the abdomen as the most severely injured body region with a frequency at 12,73%. The lowest OFI frequency was measured in the xxx cohort with a frequency at `r min.freq`%. Among the men the frequency of OFI was `r freq.ofi.men`% `r ci.men` and among the women `r freq.ofi.women`% `r ci.women`. Patients with major trauma had a higher OFI frequency compared to patient with  minor trauma. Across the most severely injured body region subgroups, OFI was most frequent among patients with the abdomen as the most severly injured body region with a frequency of 12,73%. Penetrating trauma had an OFI frequency at 6,34% and the same number for blunt trauma was 3,03%.<!-- Make sure that you present these results in the same order as you list the subgroups elsewhere-->
...
Delta values<!-- Consider calling these differences instead, so that the reader doesn't have to understand that delta values and diffferences are really the same thing--> 
------------
The largest delta value was found when comparing the Face and Abdomen <!-- I don't think that you need to capitalise the first letter in the differet subgroups-->subgroups with a delta value of 10,48 ( -1.04 - 12.88 ). The smallest delta value was found in the xxx subgruop with a delta value of ` min delta`. In the xxx subgroup the delta value with the widest range were found ` wid.delta`.<!-- Note that you need to refer to all tables in the text, you can't have tables that are not mentioned anywhere in the text. You can always write for example: "Table 4-6 shows bla bla"-->
...
The find of a higher OFI frequency in major trauma compared with minor trauma is consistent with previous research. The mortality has been found to be higher in Major trauma cohort than in Minor trauma. @Ghorbani2018-dh. What also can be seen from previous studies is a correlation between major trauma and high preventable death(10,11). That major and complex trauma has ha greater risk for non optimal trauma care was also consistent with the our results, as the median NISS was higher in the OFI cohort then in the non OFI cohort.
