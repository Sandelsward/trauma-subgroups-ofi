...
```{r main, include = FALSE, echo = FALSE, cache = TRUE}
...
patients.[@FACS; @Kadhum2020-wa] <!--Trauma system, i den generell meningen omfattar ofta inte bara koncept runt initiala omhändertagander (vilket ATLS gör). Trauma system används ofta som sam ett samlingsnamn för typ prehospital -> initial omhändertagande -> eftervårs -> +TQIP inkl trauma register osv osv. -->

ATLS is practiced in over 80 countries and 1 million doctors have gone thru 
this training.[@FACS] PTC is also used in over 80 countries, however more 
...
the PTC program is free while ATLS is not.[@Kadhum2020-wa]
 <!-- Överväger ta bort de två översta styckena helt. Vad tycker ni? 
 JA: Då de inte representerar trauma system så har jag svårt att hitta 100% plats för dem. Dessutom är iaf ATLS väldigt basic och exv KS utgår inte från det. -->
The Abbreviated Injury Scale (AIS) defines traumatic injuries across different body regions.<!--Specificera även antalet regioner, så att du kan knyta an hit i metoden -->The scale ranges from 1-5, where 1 is the least serious and 5 the most serious. [@Kuo2017-dj]

The Injury Severity Score (ISS) is an anatomic severity scale developed to assess multiple-injured patients. The ISS score a patient get is based on the AIS points from the injured body regions. The higher ISS score the more serious injury the patients suffers from. The ISS does however only take into account the most severe injury in each region, meaning that you could have two different injuries in the abdomen without it affecting the ISS score.[@Javali2019-wn]

The New Injury Severity Score (NISS) is developed from the ISS. NISS also takes injuries from the same body regions as NISS into account. The NISS is the sum of the squares of the three highest AIS codes in these regions, meaning that all three injuries can be in the same region or in separate regions. As the AIS can vary from 1-5, the NISS ranges between 1-75.[@Li2021-ag]
<!-- Definera major trauma i detta -->
...
Whether there is opportunity for improvement for a specific case or not is decided during M&M where selected trauma casees are reviewed. Opportunity for improvement (OFI) is defined as when the trauma care for a patient does not match the best practice guidelines in at least one aspect. Examples of OFI could be too long transportation time to the hospital from the site of injury, insufficient vital signs documentation at the emergency department or missing trauma surgeon/consultant. [@Sanddal2011-cl]
...
In Sweden almost two thirds of trauma patients are men and just over 60% are in the age between 18-64. For example, traumatic injuries from stabbing and from shooting violence are more common in men while women are over represented in fall traumas. There is a lot of existing data about subgroups regarding transportation time to hospital, NISS score at the Emergency department, rehabilitation, morbidity and mortality rate and so on <!--Undvik "and so on" i akademisk text om möjligt -->. [@arsrapportswetrau]

Knowledge gap 
------------

Despite having all this <!--Vilken data syftar du på ;)?. Försök skriva rakt och tydligt: Despite previous research, knowledge redarding OFI patterns among different traumating subgroups remain unknown.... Eller dylikt --> data about the trauma patients it is poorly understood whether different subgroups have greater opportunities for improvement (OFI) than others. The most common cause of death after injury is hemorrhage. Among these patients, problem in decision making, technical skills and making decisions between surgery and radiology were some of the areas were OFIs were found. <!--Hör hemma ovan bland exempel på OFI alt preventalbe death -->[@OReilly2013] However we do not know whether the same OFI areas apply for the trauma subgroups, or in what frequency. <!--Du tittar inte på vilka typer av OFI som är i grupperna varpå du inte bör nämna det här -->
...
in Solna. It is a part of SweTrau and contains 11800 patients between the years 2012 and 2021. 
The second registry is the the trauma Quality Database which is a subset of the Karolinska University Hospital Trauma Registry. It includes about 6300 patients selected for review between 2014 and 2021. The trauma care quality database includes details and conclusions from the M&M.
<!--Exakta siffror på deltagarna kan med fördel läggas under resultatet. Om du nu ändå skriver det här så var precis -->
By linking these databases together the opportunity for improvement in the trauma 
subgroups mentioned in the introduction will be assessed. <!--Denna mening tål att skrivas om -->

Setting
-------
The Karolinska University Hospital in Solna, Stockholm is the largest Trauma Center in Sweden. The hospital is also classified as a level 1 trauma center.[@Karolinska] The Trauma department at the Karolinska University Hospital in Solna handles about 1800 trauma cases every year, out of which about 200 are children. The Karolinska University Hospital is the primary trauma center for a population of 2,8 million people.[@Traumanks] 
...
<!--Stämmer inte, trauma två kommer ibland också men omhändertas av annan personal. alla regionala trauma ettor ska dock styras till KS  -->
...
<!--Registren hör rimligen till participants -->
At the Karolinska University Hospital all trauma patients are included in the Karolinska University Hospital trauma registry. The Karolinska University Hospital trauma registry is a part of the Swedish Trauma Registry, with the same eligibility criteria. <!-- Detta är för kvalitetsdatabasen och selektion till MoM. Så specificera att nedan osv hör did:  Brief review and application of audit filters by specialized nure -> review by two nurse -> MoM --> Audit filters then identify certain trauma cases in which the care might have deviated from the golden standard treatment based on different criteria. Such criteria are GCS 8 or less but the patient was not intubated, time to CT longer than 20 minutes, time to Surgery and so on <!--Undvik and so on. Alla audit filter heter VK_ något i datasetet så du kan hitta exakt där -->. Then a manual selection is done by one or two nurses, during which some cases are removed from the group of potential OFI cases because obvious reasons for the deviation can be found. The patients who are then left are discussed at a M&M where doctors and nurses from several specialties participate. At this conference every case is carefully reviewed. When OFI is found in a case, that specific OFI is described in the trauma quality database. Then it is recoded as "Yes, at least one OFI identified" or "No, no OFIs identified"
```{r, echo = FALSE} 
  ofi_selection() 
```
...
All patients from Karolinska University Hospital trauma registry and from the Trauma Quality Database. To be eligible for the study all patients had to meet the following inclusion criteria: Patients over 15 year old. No missing information in crutial colums in the registries. If information was missing for age, gender, OFI,  trauma mechanism, NISS, or AIS code the patient was excluded.

Variables and data sources/measurements
---------------------------------------
The data from the trauma registries were analyzed for OFI in separate cohorts. The cohorts were: men and women, blunt and penetrating injuries, minor and major trauma ,and across body regions injured, and trauma mechanism. The location of injuries will be analyzed based on AIS codes and AIS scores will be used to group patients into fitting cohorts. 

The patients were sorted into the the subgroups both by creating suitable columns and by using existing columns in the register file.<!--Du behöver inte beskriva att du delar in i kollumner --> Gender, trauma mechanism, age, and blunt/ penetrating trauma all had an existing column. New columns were created for OFI, minor/major trauma and for the most severe injured body region.

The column for most severely injured region was made by creating a function<!--Du behöver inte heller skriva att du skapat en funktion. Skriv att du använder AIS för att identifiera den mest skadade regionern genom att först hitta den allvarligaste skadan inkl region. Om det är flera regioner med samma skadegrad går man vidare och tittar på övriga skador inom regionen. Om det mot förmodan är exakt likadana skademönster över regionerna så kallas de multiple --> that selects the the most severely injured body region among the AIS codes belonging to a specific patient.The first digit of the AIS code represents the body region for the trauma. For patient who had two or more different injuries and thereby multple AIS codes, this function chose the most severe injury out of them, or more precisely the AIS code with the highest first digit. 

The column for Minor and Major trauma was creating using the NISS column. Patients with a NISS > 15 were defined as major trauma and those with NISS < 15 were defined as minor trauma.
...
Bias 
----
To prevent research bias all data will be scrambled while used to write the analysis model. Meaning that the real data was not available throughout the process of writing the introduction or method. The scrambled data was also used for creating functions that later could be used on the real data in the final step to get the true results. <!--Förtydliga att du först räknar ut samtliga resultat på scrambled data -->

Kanske kan lägga till något om hur jag fick tillgång till den riktiga datan när jag fått det?
<!--Behövs inte, iaf inte här -->

Study size 
----------
A complete case analysis was done on all patients in the registers that met the inclusion criteria. <!--Helt rätt. Tillse att du verkligen tydlig skrivit "the inclusion criteria for ..." ovan --> This resulted in a cohort of 2696 patients. <!--Behöver inte nämna exakt antal här -->

Quantitative variables
----------------------

The registers hold both categorical and contunious variables. NISS, age and AIS codes were treated as continious variables. Penetraiting/ Blunt Trauma, Minor/ Major Trauma and OFI was treated as binary categorical variables. Trauma mechanism and Most severe region of injury was treated as categorical variabels.

The range of NISS was 1-75, representing the number of AIS points for each patient. Age varied from 15 to 102.<!--Just spridningsmåtten för uppenbara variabler som ålder behöver du inte redovisa. --> Penetraiting/ Blunt Trauma had the values of 1 and 2. 1 represented Blunt trauma and 2 penetraiting trauma. Minor/Major was derived from the NISS value. NISS>15 was considered a Major Trauma and <15 a Minor Trauma. OFI had the values of either YES or NO.


Trauma Mechanism hade a value that ranged from 1-13. Each number was represented by the following trauma mechanism:
1: Trauma mechanism Traffic - motor veichle accident, not motorcycle
2: Traffic - motorcycle accident	
3: Traffic - bicycle accident	
4: Traffic - pedastrian	
5: Traffic - other	
...
9: Low energy fall - fall in the same level	
10: High energy fall - fall from a higher level
11: Blast injury	
12: Other	
13: Unknown

The most severe region had 10 different values: head, neck, face, thorax, upper extremity, lower extremity, abdomen, spine, multiple and unspecified. These 10 values were regrouped according to the ISS system as following. <!--Du kan slå ihop dessa listor till flytande text. -->

1: Head and neck
2: Face
3: Chest, diaphragm or spine
4: Abdomen
5: Extremities
6: Unspecified
7: Multiple
...
Statistical methods
-------------------
For handling the data and making the statistical analysis the program R was used<!--Källa -->. R is a programming language used for statistical computing. The unfiltered data sets were uploaded to Rstudio which is an integrated development environment for R.<!--Behöver inte nämna --> Then all data that was not needed for this study was removed, including columns with non essential information, patients without complete data, and those who did not meet up with the inclusion criteria. New columns were also added as listed above. 
...
For estimating the confidence interval Bootstrapping was used on the data. To be able to do the Bootstrapping <!--Vi behöver inte beräkna deltavärden för att kunna göra bootstrap. Du behöver göra en bootstrap för att för CI på dina resultat, inkl deltavärdena --> the absolute difference in OFI frequency within each subgropu was calculated. Meaning that the absolute difference between men and women was calculated but not for example between women and blunt trauma. 1000 boots will be done, creating 1000 fiqtive ??? on which the code will be tested. <!--Läs på vad bootstrap är. Du simulerar fram nya dataset och gör om dina beräkningar -> går att räkna CI -->
...
After merging the Karolinska trauma registry and the Trauma quality data base there was `r all.patients` patients. After applying the inclusion criteria the number of patients was reduced to `r total.cohort`. <!--Bra att du använder objekt -->
...
Out of the `r total.cohort` patients included in the cohort `r all.men` were men and `r total.women` were women. <!--Du behöver inte säga vad den "andra" gruppen är. Exv om 60% är män fattar man att 40% är kvinnor. --> Among the men the frequency of OFI was `r freq.ofi.men` and among the women `r freq.ofi.women`. 
<!--Förslag vad som ska stå här: Vanligaste kön, genomsnittlig ålder, genomsnitts ISS eller niss. Du bör också presentera hur vanligt OFI var osv. -->

Skriva ett objekt för den som hade högst frekvens av ofi
...
Main results
------------
The frequency of OFI for the different subgroups were as following:
