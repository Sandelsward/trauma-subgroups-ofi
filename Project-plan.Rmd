...
The Abbreviated Injury Scale (AIS) defines traumatic injuries across different body regions.The scale ranges from 1-5, where 1 is the least serious and 5 the most serious. [@Kuo2017-dj]
...
Study size 
----------
A complete case analysis was done on all patients in the registers that met the inclusion criteria. This resulted in a cohort of 2696 patients.
...
The most severe region had 10 different values: head, neck, face, thorax, upper extremity, lower extremity, abdomen, spine, multiple and unspecified. These 10 values were regrouped according to the ISS system as following.
...
Statistical methods
-------------------
For handling the data and making the statistical analysis the program R was used. R is a programming language used for statistical computing. The unfiltered data sets were uploaded to Rstudio which is an integrated development environment for R. Then all data that was not needed for this study was removed, including columns with non essential information, patients without complete data, and those who did not meet up with the inclusion criteria. New columns were also added as listed above. 
...
For estimating the confidence interval Bootstrapping was used on the data. To be able to do the Bootstrapping the absolute difference in OFI frequency within each subgropu was calculated. Meaning that the absolute difference between men and women was calculated but not for example between women and blunt trauma. 1000 boots will be done, creating 1000 fiqtive ??? on which the code will be tested.
...
Out of the `r total.cohort` patients included in the cohort `r all.men` were men and `r total.women` were women. Among the men the frequency of OFI was `r freq.ofi.men` and among the women `r freq.ofi.women`. 
